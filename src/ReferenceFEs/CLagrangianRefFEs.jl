...
Pushforward(::Type{Lagrangian}, ::GradConformity) = IdentityPiolaMap()
...
"""
    ReferenceFE{N}(reffe::GenericLagrangianRefFE{GradConformity}, iface::Integer)
"""
...
Vector of the reference FE of each `d`-face of `reffe`'s polytope. This
corresponds to the unique refference FEs that were generated by
[`compute_lagrangian_reffaces`](@ref) at the creation of `reffe`.
"""
...
Builds a `LagrangianRefFE` object on top of `p`. `T` is the type of the value of
the approximation space (e.g., `T=Float64` for scalar-valued problems,
`T=VectorValue{N,Float64}` for vector-valued problems with `N` components).

The arguments `order` and `orders` are for the polynomial order of the resulting
space, which allows isotropic or anisotropic orders respectively (provided that
the cell topology allows the given anisotropic order). The argument `orders`
should be an indexable collection of `D` integers.
...
of `p` when restricting the order per direction `orders` to this face.
...
Returns `node_coords`, the nodal coordinates of all the Lagrangian nodes
associated with the order per direction `orders`, and `face_own_nodes`, a vector
of vectors indicating which nodes are owned by each of the faces of `p`.
"""
...
Returns a vector of vectors with the permutations of the nodes owned by the
interior of `p`, see also [`get_face_own_nodes_permutations`](@ref).
"""
...
Returns a tuple of length `D`. Its `d+1` entry is a vector of `LagrangianRefFE`,
one for each face of dimension `d` on the boundary of `p`.

These refference FEs can be used as a geometrical mappings from the reference
polytope of the corresponding face of `p` into the face itself, or the face
mapped in the physical space by the shape functions of
`LagrangianRefFE(T,p,orders;space=space)` as geometrical map.
"""
